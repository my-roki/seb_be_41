...
public class Q_07_convertListToObject {
    public static void main(String[] args) {
        // write test case here
        Q_07_convertListToObject coplit = new Q_07_convertListToObject();
        String[][] arr = new String[][]{{"make", "Ford"}, {"model", "Mustang"}, {"year", "1964"}, {"make", "Bill"}};

        HashMap<String, String> output = coplit.convertListToHashMap(arr);
        System.out.println(output); // --> { "make" = "Ford", "model" = "Mustang", "year" = "1964"}
    }

    public HashMap<String, String> convertListToHashMap(String[][] arr) {
        HashMap<String, String> output = new HashMap<>();
        for (String[] kv : arr) {
            if (kv.length != 0 && !output.containsKey(kv[0])) {
                output.put(kv[0], kv[1]);
            }
        }
        return output;
    }
